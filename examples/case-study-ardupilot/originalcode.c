 ...
 *
 * Created by julien on Thu Sep 24 11:42:34 2009
 */
...
#define max16_throttle                                                         \
  2100 // ESC max position, given in useconds, in my ungly servos 2100 is fine,
       // you can try 2000..
...
byte current_wp =
    1; // This variables stores the actual waypoint we are trying to reach..
byte jumplock_wp =
    0; // When switching waypoints this lock will allow only one transition..
byte wp_home_lock = 0;        //
...
// ACME variables
byte gps_new_data_flag =
    0; // A simple flag to know when we've got new gps data.
unsigned int launch_altitude = 0; // launch altitude, altitude in waypoints is
                                  // relative to starting altitude.
int middle_thr = 90;              // The central position
int middle_yaw = 90;              // The cnetral position of yaw
byte middle_measurement_lock =
    0; // Another lock to void resetting the middle measurement..
/*******************************/
...
void setup() {
  ...
  // Testing servos (max and mins), we are aware of the propeller and won't spin
  // that without warning... =) test_throttle();
  test_yaw();
...
void loop() {
  /*************************************************************************
   * GPS function, reads and update all the GPS data... This function is located
   *in "GPS_Navigation" tab
   *************************************************************************/
  gps_parse_nmea();

  /*************************************************************************
   *************************************************************************/
  if ((gps_new_data_flag & 0x01) ==
      0x01) // Checking new GPS "GPRMC" data flag in position
  {
    ...
    if (get_gps_dist(lat, lon, wp_lat[0], wp_lon[0]) >
        distance_limit) // If the distance from home is greater than 2000 meters
    {
  ...
  /*************************************************************************/
  /*This is just a prototype, ensure that the autopilot will jump ONLY ONE
   * waypoint, checks RTL mode */
  /*************************************************************************/
  if (RTL == 0) // Verify the RTl option (0=Waypoint mode, 1 = Return Home mode)
  {
    if ((wp_distance < 30) &&
        (jumplock_wp == 0x00)) // Checking if the waypoint distance is less than
                               // 30, and check if the lock is open
    {
      current_wp++;               // Switch the waypoint
      jumplock_wp = 0x01;         // Lock the waypoint switcher.
      if (current_wp > waypoints) // Check if we've passed all the waypoints, if
                                  // yes will return home..
      {
        current_wp = 0;
      }
    }

    if (digitalRead(4) == LOW) // Checks the MUX pin to see if we are in manual
                               // mode (Low = manual)
    {
      reset_PIDs(); // Reset all the PIDs
      middle_measurement_lock =
          0; // status flag, to lock the starting position of the throttle
    }
  ...
  } else // RTL set
  {
    if (digitalRead(4) == LOW) // Checks the MUX pin to see if we are in manual
                               // mode (Low = manual)
    {
      wp_alt[0] = alt; // updates the altitude till we switch to automode, then
                       // the autopilot will try to mantain that altitude
      reset_PIDs();    // Reset all the PIDs
      middle_measurement_lock =
          0; // unlocks the starting positions of throttle
             // and yaw. See the function in "AutoPilot"
             // tab, inside the throttle_control() function
    }
...
void send_to_ground(void) {
  // Warning if you are using a baud rate of 4800 this will super slow down the
  // system..
  Serial.print("Mot: ");
  Serial.print(test3);
  Serial.print(" Yaw: ");
...
/**************************************************************
 * Special module to limit float values, which is like constrain() but for
 *variables IEEE 754 (Don't worry about it, it's just a floating point
 *variables). =) It's called "ACME" because that's the generic name for
 *everything in the Road Runner cartoons ;-)
 ***************************************************************/
...
/***************************************************************************/
// Computes heading the error, and choose the shortest way to reach the desired
// heading
/***************************************************************************/
int compass_error(int PID_set_Point, int PID_current_Point) {
...
/*************************************************************************
 * Throttle Control, reads gps info, executes PID and pulses the motor
 *controller..
 *************************************************************************/
void throttle_control(void) {

  if ((middle_measurement_lock == 0) &&
      (digitalRead(4) == HIGH)) // Verify if the lock is open (equal to zero)
                                // and if we are in automode..
  {
    ...
    middle_thr =
        ...
        (max16_throttle - min16_throttle); // Converting the pulse to degrees...
    ...
    read_servo =
        pulseIn(3, HIGH); // Reads the pulse length of signal from receiver
    middle_yaw = ((read_servo - min16_yaw) * 180L) /
                 (max16_yaw - min16_yaw); // Converting the pulse to degrees...
    Serial.println(read_servo);           // Just print values
    Serial.println(middle_yaw);
  }

  // Central Position + PID(current altitude, desired altitude - launch
  // altitude). The result will be the motor speed value.. I subtract the
  // current altitude (alt) from the "launch_altitude" in order to make it
  // realtive to the start point, Example: If you launch you airp. at 300 meters
  // above sea level, and you want to maintain 50 meters above your head, the
  // airp. will fly at 350 meters above sea level.
  test3 =
      middle_thr + PID_altitude(wp_alt[current_wp], (alt - launch_altitude));
...
/*************************************************************************
 * Yaw Control, reads gps info, calculates navigation, executes PID and sends
 *values to the servo..
 *************************************************************************/
void yaw_control(void) {

  wp_bearing = get_gps_course(
      lat, lon, wp_lat[current_wp],
      wp_lon[current_wp]); // Calculating Bearing, this function is located in
                           // the GPS_Navigation tab..
  wp_distance = get_gps_dist(
      lat, lon, wp_lat[current_wp],
      wp_lon[current_wp]); // Calculating Distance, this function
                           // is located in the GPS_Navigation tab..

  // test=middle_yaw+PID_heading(compass_error(wp_bearing, course)); //Central
  ...
  pulse_servo_yaw(
      test); // Sending values to servo, 90 degrees is central position.
}

/*************************************************************************
 * This functions parses the NMEA strings...
 * Pretty complex but never fails and works well with all GPS modules and baud
 *speeds.. :-) Just change the Serial.begin() value in the first tab for higher
 *baud speeds
 *************************************************************************/
    ...
    /*************************************************/
      ...
      if (buffer[counter] == 0x0A) // Looks for \F
      {
        ...
        if (strncmp(buffer, head_rmc, 5) == 0) // looking for rmc head....
        {
          ...
          for (int x = 0; x < 100; x++) {
            if (buffer[x] == '*') {
              checksum_received = strtol(&buffer[x + 1], NULL,
          ...
          {
            /* Token will point to the data between comma "'", returns the data
             * in the order received */
            /*THE GPRMC order is: UTC, UTC status ,Lat, N/S indicator, Lon, E/W
             * indicator, speed, course, date, mode, checksum*/
            token = strtok_r(buffer, search,
                             &brkb); // Contains the header GPRMC, not used
            ...
            // Longitude in degrees, decimal minutes. (ej. 4750.1234 degrees
            // decimal minutes = 47.835390 decimal degrees) Where 47 are degrees
            // and 50 the minutes and .1234 the decimals of the minutes. To
            // convert to decimal degrees, devide the minutes by 60 (including
            // decimals), Example: "50.1234/60=.835390", then add the degrees,
            // ex: "47+.835390=47.835390" decimal degrees
            token = strtok_r(
                NULL, search,
                &brkb); // Contains Latitude in degrees decimal minutes...

            // taking only degrees, and minutes without decimals,
            // strtol stop parsing till reach the decimal point "."  result
            // example 4750, eliminates .1234
            ...
            // joining degrees, minutes, and the decimals of minute, now without
            // the point... Before was 4750.1234, now the result example is
            // 47501234...
            temp3 = (temp * 10000) + (temp2);

            // modulo to leave only the decimal minutes, eliminating only the
            // degrees.. Before was 47501234, the result example is 501234.
            temp3 = temp3 % 1000000;
            ...
            // First i convert the decimal minutes to degrees decimals stored in
            // "temp3", example: 501234/600000= .835390 Then i add the degrees
            // stored in "temp" and add the result from the first step, example
            // 47+.835390=47.835390 The result is stored in "lat" variable...
            ...
            gps_new_data_flag |=
                0x01; // Update the flag to indicate the new data has arrived.
        ...
        if (strncmp(buffer, head_gga, 5) == 0) // now looking for GPGGA head....
        {
          ...
          if (checksum_received == checksum) // Checking checksum
          {
            ...
            token = strtok_r(buffer, search,
                             &brkb); // GPGGA header, not used anymore
            ...
            token = strtok_r(NULL, search,
                             &brkb); // ALTITUDE, is the only meaning of this
            ...
            if (fix_position >= 0x01)
              digitalWrite(12, HIGH); // Status LED...
            else
              digitalWrite(12, LOW);

            gps_new_data_flag |=
                0x02; // Update the flag to indicate the new data has arrived.
          }
...
void init_ardupilot(void) {
  ...
  // This next line is a Mode pin, which only works in with three-position
  // toggle switches on your transmitter. If you want to use it for some special
  // mode, you must change the Attiny code. It is not currently active. When you
  // put the switch in the central position the attiny will set high a pin
  // called "mode", and you can use it to do whatever yowant...
  pinMode(5, INPUT);   // Mode pin (see above)
  pinMode(11, OUTPUT); // Simulator Output pin
  pinMode(12,
          OUTPUT); // LOCK LED pin in ardupilot board, indicates valid GPS data
  pinMode(13, OUTPUT); // STATS LED pin in ardupilot board, blinks to indicate
                       // the board is working well...
}

void init_startup_parameters(void) {
  // yeah a do-while loop, checks over and over again until we have valid GPS
  // position and lat is diferent from zero. I re-verify the Lat because
  // sometimes fails and sets home lat as zero. This way never goes wrong..
  do {
  ...
  } while ((gps_new_data_flag & 0x01 != 0x01) &
           (gps_new_data_flag & 0x02 != 0x02));

  yaw_control(); // I've put this here because i need to calculate the distance
                 // to the next waypoint, otherwise it will start at waypoint 2.

  launch_altitude = alt; // Saving the launch altitude, altitude in waypoints is
                         // relative to starting altitude.
  ...
  wp_alt[0] =
      100; // Storing home altitude plus 100, otherwise it will try to maintain
           // the launch altitude, you really don't want fly too low..
...
int PID_heading(int PID_error) {
  ...
  float dt =
      (float)(millis() - heading_PID_timer) /
      1000; // calculating dt, you must divide it by 1000, because this system
            // only undestands seconds.. and is normally given in millis

  // Integratior part
  heading_I +=
      (float)PID_error * (float)dt; // 1000 microseconds / 1000 = 1 millisecond
  heading_I = constrain(heading_I, heading_min,
                        heading_max); // Limit the PID integrator...
  ...
  heading_output =
      (kp[0] * PID_error); // Proportional part, is just the KP constant *
                           // error.. and addidng to the output
  ...
  heading_output = constrain(heading_output, heading_min,
                             heading_max); // limiting the output....

  heading_previous_error = PID_error; // Saving the actual error to use it later
                                      // (in derivating part)...
...
int PID_altitude(int PID_set_Point, int PID_current_Point) {
  static unsigned int altitude_PID_timer; // Timer to calculate the dt of the
                                          // PID
  ...
  float dt =
      (float)(millis() - altitude_PID_timer) /
      1000; // calculating dt, you must divide it by 1000, because this system
            // only undestand seconds.. and is normally given in millis
  ...
  altitude_output = constrain(altitude_output, altitude_min,
                              altitude_max); // PID_P+PID_I+PID_D
  altitude_previous_error = PID_error; // Saving the actual error to use it
                                       // later (in derivating part)...
  altitude_PID_timer = millis(); // Saving the last execution time, important to
                                 // calculate the dt...
...
/**************************************************************
 * Configuring the PWM hadware... If you want to understand this you must read
 *the Data Sheet of atmega168..
 ***************************************************************/
void Init_servo(void) // This part will configure the PWM to control the servo
                      // 100% by hardware, and not waste CPU time..
{
  ...
  // predefined Atmega168 registers. We load the registers and the chip does the
  // rest.

  // Remember the registers not declared here remains zero by default...
  TCCR1A = ((1 << WGM11) | (1 << COM1B1) |
            (1 << COM1A1)); // Please read page 131 of DataSheet, we are
                            // changing the registers settings of
                            // WGM11,COM1B1,COM1A1 to 1 thats all...
  TCCR1B = (1 << WGM13) | (1 << WGM12) |
           (1 << CS11); // Prescaler set to 8, that give us a resolution of 2us,
                        // read page 134 of data sheet
  OCR1A = 2500; // the period of servo 1, remember 2us resolution, 2500/2 =
                // 1250us the pulse period of the servo...
  OCR1B = 3000; // the period of servo 2, 3000/2=1500 us, more or less is the
                // central position...
  ICR1 = 40000; // 50hz freq...Datasheet says  (system_freq/prescaler)/target
                // frequency. So (16000000hz/8)/50hz=40000, must be 50hz because
                // is the servo standard (every 20 ms, and 1hz = 1sec)
                // 1000ms/20ms=50hz, elementary school stuff...
}
/**************************************************************
 * Function to pulse the throttle servo
 ***************************************************************/
void pulse_servo_throttle(
    long angle) // Will convert the angle to the equivalent servo position...
{
  // angle=constrain(angle,180,0);
  OCR1A =
      ((angle * (max16_throttle - min16_throttle)) / 180L + min16_throttle) *
      2L;
}
 ...
 ***************************************************************/
void pulse_servo_yaw(
    long angle) // Will convert the angle to the equivalent servo position...
{
  // angle=constrain(angle,180,0);
  OCR1B =
      ((angle * (max16_yaw - min16_yaw)) / 180L + min16_yaw) * 2L; // Scaling
}
/**************************************************************
...
// Same as pulseIn, but tweaked for range 1000 - 2000 usec, and reading only
// HIGH phase. Must be compiled in .cpp file, with -Os compiler switch.
unsigned long PulseIn(uint8_t pin, unsigned long timeout) {
