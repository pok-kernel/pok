...
void *pingme_job() {
  ...
  pok_time_t mytime;
  bool_t valid;
  ret = pok_port_sampling_create("datain", 4, POK_PORT_DIRECTION_IN, 10,
  ...
  while (1) {
    ...
    ret = pok_port_sampling_read(port_pingme_data_sink, &data_sink_request_var,
                                 &length, &valid);
    if (ret == 0) {
    ...
    pok_thread_sleep_until(mytime * 1000);
  }
}
