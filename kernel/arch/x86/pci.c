...
static inline unsigned int pci_read(unsigned int bus, unsigned int dev,
                                    unsigned int fun, unsigned int reg) {
  unsigned int addr =
      (1 << 31) | (bus << 16) | (dev << 11) | (fun << 8) | (reg & 0xfc);
  unsigned int val = -1;
...
static inline unsigned int pci_read_reg(s_pci_device *d, unsigned int reg) {
  ...
  for (bus = 0; bus < PCI_BUS_MAX; bus++)
    for (dev = 0; dev < PCI_DEV_MAX; dev++)
      for (fun = 0; fun < PCI_FUN_MAX; fun++)
        if (((unsigned short)pci_read(bus, dev, fun, PCI_REG_VENDORID)) ==
                d->vendorid &&
            ((unsigned short)pci_read(bus, dev, fun, PCI_REG_DEVICEID)) ==
                d->deviceid) {
...
void dummy_pci_handler(void) {
  __asm__ volatile(".globl pci_handler\n"
                   "pci_handler:\n"
...
pok_ret_t pci_register(s_pci_device *dev, uint8_t part_id) {
  ...
  pok_idt_set_gate(32 + dev->irq_line, GDT_CORE_CODE_SEGMENT,
                   (uint32_t)pci_handler, IDTE_INTERRUPT, 0);
