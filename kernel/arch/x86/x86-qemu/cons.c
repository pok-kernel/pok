...
#define COM1 0x3F8
    ...
    defined(POK_NEEDS_COVERAGE_INFOS) || defined(POK_NEEDS_USER_DEBUG)

int is_transmitter_holding_register() { return inb(COM1 + 5) & 0x20; }
...
int is_received_data_ready() { return inb(COM1 + 5) & 0x01; }
...
void pok_cons_print_char(const char c) {
  ...
  if (local_curs.row >= screen_h) {
    ...
    for (i = 0; i < screen_w; ++i) {
      ...
      *(vga_base + 2 * screen_w * (screen_h - 1) + 2 * i + 1) =
          local_curs.cur_attr;
    }
...
#ifdef POK_NEEDS_CONSOLE
void pok_cons_get_char(char *c) { read_serial(c); }
#endif

void pok_cons_attr(uint8_t attr) { g_cons.cur_attr = attr; }
...
void pok_cons_clear(void) {
  ...
  char *ptr;
  static struct s_cons
      local_curs; /* local copy of the current cursor position */
...
#if defined(POK_NEEDS_CONSOLE) || (POK_NEEDS_DEBUG) ||                         \
    defined(POK_NEEDS_INSTRUMENTATION) || defined(POK_NEEDS_COVERAGE_INFOS) || \
