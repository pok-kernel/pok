...
#ifndef POK_CONFIG_NB_INPUT_VIRTUALPORT
#define POK_CONFIG_NB_INPUT_VIRTUALPORT 20
#endif
...
typedef struct {
  ...
  unsigned int addr;
  char mac[6];
  pok_queue_t recv_buf[POK_CONFIG_NB_INPUT_VIRTUALPORT];
  uint32_t msg_nb;
} s_ne2000_dev;
...
#define NE2000_CR_STP                                                          \
  (1 << 0) /* This bit is the STOP command. When it is                         \
              set, no packets will be received or                              \
              transmitted. POWER UP=1. */
#define NE2000_CR_STA                                                          \
  (1 << 1) /* The STA bit controls nothing. It only                            \
              reflects the value written to this bit.                          \
              POWER UP=0. */
#define NE2000_CR_TXP                                                          \
  (1 << 2)                     /* This bit must be set to transmit a packet.   \
...
#define NE2000_ISR_PRX                                                         \
  (1 << 0) /* This bit indicates packet received with no                       \
      errors. */
#define NE2000_ISR_PTX                                                         \
  (1 << 1) /* This bit indicates packet transmitted with                       \
      no errors. */
#define NE2000_ISR_RXE                                                         \
  (1 << 2) /* This bit is set when a packet received                           \
      with one or more of the following errors:                                \
        - CRC error                                                            \
        - Frame alignment error                                                \
        - Missed packet */
#define NE2000_ISR_TXE                                                         \
  (1 << 3) /* Transmit error bit is set when a packet                          \
      transmission is aborted due to excessive                                 \
      collisions. */
#define NE2000_ISR_OVW                                                         \
  (1 << 4) /* This bit is set when the receive buffer                          \
      has been exhausted. */
#define NE2000_ISR_CNT                                                         \
  (1 << 5) /* Set when MSB of one or more of the network                       \
      tally counters has been set. */
#define NE2000_ISR_RDC                                                         \
  (1 << 6) /* Set when remote DMA operation has been                           \
      completed. */
#define NE2000_ISR_RST                                                         \
  (1 << 7) /* This bit is set when NIC enters reset state                      \
...
#define NE2000_DCR_WTS                                                         \
  (1 << 0) /* Word Transfer Select                                             \
        0: byte-wide DMA transfer                                              \
        1: word-wide DMA transfer */
#define NE2000_DCR_BOS                                                         \
  (1 << 1) /* Byte Order Select                                                \
        0: MS byte placed on MD15-8 and LS byte on MD7-0. (32xxx,80x86)        \
        1: MS byte placed on MD7-0 and LS byte on MD15-8. (680x0) */
#define NE2000_DCR_LAS                                                         \
  (1 << 2) /* This bit must be set to zero. NIC only supports dual 16-bit DMA  \
      mode. POWER UP =1 */
#define NE2000_DCR_LS                                                          \
  (1 << 3) /* Loopback Select                                                  \
                0: Loopback mode selected. Bits 1 and 2 of the TCR must also   \
              be programmed for Loopback operation. 1: Normal Operation */
#define NE2000_DCR_ARM                                                         \
  (1 << 4)                      /* Auto-initialize Remote                      \
...
#define NE2000_RCR_AB                                                          \
  (1 << 2) /* If AB=1, packets with broadcast destination                      \
...
#define NE2000_RSR_PRX                                                         \
  (1 << 0) /* This bit indicates packet received with                          \
      no errors.*/
...
#define NE2000_RXBUF                                                           \
  (NE2000_TXBUF + NE2000_TXBUFSZ) // rx buf offset
                                  // in 256 bytes block
#define NE2000_RXBUFSZ                                                         \
  (NE2000_MEMSZ - NE2000_TXBUFSZ) // rx buf size
                                  // in 256 bytes block
