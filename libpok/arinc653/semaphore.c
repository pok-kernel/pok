...
#define CHECK_SEM_INIT                                                         \
  if (pok_arinc653_semaphores_initialized == 0) {                              \
    uint16_t bla;                                                              \
...
pok_arinc653_semaphore_layer_t
    pok_arinc653_semaphores_layers[POK_CONFIG_ARINC653_NB_SEMAPHORES];
...
void CREATE_SEMAPHORE(SEMAPHORE_NAME_TYPE SEMAPHORE_NAME,
  ...
  *RETURN_CODE = INVALID_CONFIG;
  ...
  core_ret =
      pok_sem_create(&sem_id, CURRENT_VALUE, MAXIMUM_VALUE, QUEUING_DISCIPLINE);
...
void WAIT_SEMAPHORE(SEMAPHORE_ID_TYPE SEMAPHORE_ID, SYSTEM_TIME_TYPE TIME_OUT,
                    RETURN_CODE_TYPE *RETURN_CODE) {
  pok_ret_t core_ret;
...
void SIGNAL_SEMAPHORE(SEMAPHORE_ID_TYPE SEMAPHORE_ID,
  ...
  core_ret =
      pok_sem_signal(pok_arinc653_semaphores_layers[SEMAPHORE_ID].core_id);
