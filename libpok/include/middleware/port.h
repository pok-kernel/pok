...
#define pok_port_queueing_status(id, status)                                   \
  pok_syscall2(POK_SYSCALL_MIDDLEWARE_QUEUEING_STATUS, (uint32_t)id,           \
               (uint32_t)status)
/*
 * Similar to:
 * pok_ret_t pok_port_queueing_status (const pok_port_id_t id, const
...
pok_ret_t pok_port_sampling_read(const pok_port_id_t id, void *message,
...
#define pok_port_sampling_status(id, status)                                   \
  pok_syscall2(POK_SYSCALL_MIDDLEWARE_SAMPLING_STATUS, (uint32_t)id,           \
               (uint32_t)status)
/*
 * Similar to:
 * pok_ret_t pok_port_sampling_status (const pok_port_id_t id, const
