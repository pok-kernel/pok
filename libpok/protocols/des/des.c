...
void pok_protocols_des_marshall(void *uncrypted_data, pok_size_t uncrypted_size,
  ...
  DES_ncbc_encrypt(uncrypted_data, crypted_data, uncrypted_size, &schedule,
                   &ivec, DES_ENCRYPT);
  *crypted_size = 8;
}

void pok_protocols_des_unmarshall(void *crypted_data, pok_size_t crypted_size,
                                  void *uncrypted_data,
  ...
  memcpy(ivec, initVector, sizeof(initVector));
  DES_ncbc_encrypt(crypted_data, uncrypted_data, crypted_size, &schedule, &ivec,
                   DES_DECRYPT);
